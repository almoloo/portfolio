...
**Portfolio thumbnails — project logos, uniform dimensions** - replaced screenshot thumbnails with each project's actual logo/icon normalized to identical 512×512 PNGs, with a detail-panel image treatment so logos display intact; plus follow-on refinements in the same branch: removed demo links for 5 projects that don't have a real one, recolored/repadded the flow, lucid, bellman, and wallet-algorand logos, embedded real GitHub screenshots into 11 project write-ups, fixed a scoped-CSS bug that silently no-opped styling on markdown-rendered content, and loosened detail-panel spacing (Completed)
